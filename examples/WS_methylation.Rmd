...
The data are compound of two more element:
* The base pair position of each CpG
* The individual CpG level (in M values)
```{r}
temp.pos[1:10]#the base pair position of each CpG
length(temp.pos)#total number of CpG
...
Here we divide the data in regions that have at least 17 CpGs separated by a maximum distance of 500 bp. This results to a missigness rate of 1.89 %
```{r }
thresh <- 500
tl <- split(temp.pos , cumsum(c(1, diff(temp.pos) > thresh)  ) ) #Remove case with too long distance

#Level of analysis 4
tl <-  tl[-which(lengths(tl)< 17)]#List of the position of CpG that we will later analyse
sum(lengths(tl))/length( temp.pos)#percent missigness
hist( lengths(tl),nclass = 100, main="Histogram of the number of CpG per regions")

```


## Running Wavelet Screaming region by region

In this section we run Wavelet Screaming on each regions speratly. This takes about 3/4 minutes when using a single CPU. 
```{r message=FALSE}
...
{
  reg_mat <- sim_methy_all[which( temp.pos  %in% tl[[i]]),] #Select CpG in region i
  bp <- temp.pos[which( temp.pos  %in% tl[[i]])]#Correspond to the base position of the CpG of region i 
...
We concatenate the results of Wavelet Screaming in one data frame. 
```{r echo=TRUE}
...
In order to assess the significance of each regions, we simulate the null distribution of the test statistic. Below we display how to simulate this distribution
```{r ,message=FALSE,}
set.seed(1)
#Simulation function using the same parameter as used in the Wavelet_screaming function
tt <- proc.time()
Sim <- Simu_null_emp(res=res,
                       coeftype="c",
                       lev_res = 4,
                       base_shrink = 1/26,
                       size=100000#Number of simulation required
                     )
proc.time()  -tt #total running time
plot( Sim[,1], Sim[,2])
```
...
As explain in the article, the distribution of L~h~ might not be Gaussian when analysing data using low depth. Below, we display how to perform a Box-Cox transform on the simulations of the test statistic under the H~0~ .
...
When the Box-Cox transform is done, we can then compute the pvalue of each region as follow.

```{r,message=FALSE }
lambda <- Search_lambda(Sim,plot=TRUE)#optimizing the lambda* value

Th <- Sim[,c("L_h")]+lambda*Sim[,c("min_ph_pv")] #Computing the test statistic null distribution
...
Here we see the null distribution of the test statistic and on the right side of the plot the associated regions.

```{r,message=FALSE }
#Computing the p-value
pv <- 1-pnorm(th,mean=muv,sd=sdv)
hist(1-pnorm(th,mean=muv,sd=sdv),nclass=100, main="Histogramm of the pvalues")
hist(log10(1-pnorm(th,mean=muv,sd=sdv)),main ="Histogramm of the Log10 pvalues")
```
...
head(sub) #data frame with starting and ending position of each DMR we are trying to detect.
...
sum( lengths(lstemp)) # number of CpG to detect
...
If a region contains more than one CpG that is differentially methylated then it has to be deteceted
```{r }
...
df_out summaries the number of regions detected using different criterion as well as the number of truly associated regions.
