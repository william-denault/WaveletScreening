---
title: "Wavelet screening on methylation"
...
In this section, we run Wavelet screening on each region separately. This takes about 3/4 minutes when using a single CPU. 
```{r message=FALSE}
...
We concatenate the results of Wavelet screening in one data frame. 
```{r echo=TRUE}
...
Here we have an additionnal issue, the observed test statistics contains some positive and negative values. However the Box-Cox transform requires to have only positive values. We therefore shift the distribution and then apply the Box-Cox transform
...
When the Box-Cox transform is done, we can then compute the p-value of each region as follow.
```{r,message=FALSE }
...
hist(log10(1-pnorm(th,mean=muv,sd=sdv)),nclass=100,main ="Histogramm of the Log10 pvalues")
```
...
df_out summaries the number of regions detected using different criteria and the number of truly associated regions.
