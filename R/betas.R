#'@title Compute Beta value from Frequentist or Bayesian linear regression
...
#'of the computation can be found in our paper "Wavelet screening: a novel look to GWAS data"
...
betas <- function(y,x,confounder, sigma_b=NA,all)
{
  ...
  if( is.na(sigma_b))
  {
    ...
    Dmat <- as.matrix(Dmat)
    index <- pc+1
    betas <- solve(iag(1/sigma_b/sigma_b,index)) %*% t(Dmat)%*% y
    colnames(betas) <- "Beta"
  }
  if(!is.na(sigma_b))
  {
